 ...
 * ORIG-DATE:    Sep-02
 * Last Change: Wed Jun 18 11:46:11 2003 by Ed Maurer <edm@u.washington.edu>
...
void InitChannelSediment(Channel * Head)
{
  ...
  int i;
  float InitialDepth = 0.01; /* initial depth of sediment in the channel, m */
...
void RouteChannelSediment(Channel * Head, Channel *RoadHead, TIMESTRUCT Time, DUMPSTRUCT *Dump)
{
  ...
  for (order = 1;; order += 1) {
	...
	for(i=0;i<NSEDSIZES;i++) {
	  ...
	  /* sed velocity per Wicks and Bathurst */
	  Vsed = 8.5*Vshear*sqrt(1-(Vshearcrit/Vshear));
	  if(Vsed>V) Vsed=V;
	  Current->sediment.outflowconc = 
	    (1000.0*sumoutflow/(Current->outflow))*(V/Vsed);
